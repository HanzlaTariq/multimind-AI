...
import ToolCreditConfig from "../models/ToolCreditConfig.js";
...
// Seed values — only used the very first time the app runs (to populate the
// ToolCreditConfig collection) and as a last-resort fallback if the DB is
// ever unreachable. Once seeded, the ADMIN PANEL (Admin → Tool Costs) is the
// source of truth: editing a tool's cost there — either right away, or
// scheduled for a future date — is what actually changes what gets charged.
export const TOOL_COST_SEED = [
  { toolId: "compress-pdf", label: "Compress PDF", costType: "fixed", cost: 1 },
  { toolId: "convert-image", label: "Convert image", costType: "fixed", cost: 1 },
  { toolId: "convert-spreadsheet", label: "Convert spreadsheet", costType: "fixed", cost: 1 },
  { toolId: "convert-document", label: "Convert document", costType: "fixed", cost: 2 },
  { toolId: "merge-pdf", label: "Merge PDF", costType: "fixed", cost: 1 },
  { toolId: "split-pdf", label: "Split PDF", costType: "fixed", cost: 1 },
  { toolId: "images-to-pdf", label: "Images to PDF", costType: "fixed", cost: 1 },
  { toolId: "pdf-to-images", label: "PDF to images", costType: "fixed", cost: 2 },
  {
...
let toolCostCache = global.__toolCostCache;
if (!toolCostCache) {
  toolCostCache = global.__toolCostCache = { costs: null, loadedAt: 0 };
}

async function seedToolCostsIfEmpty() {
  const count = await ToolCreditConfig.countDocuments();
  if (count > 0) return;
  await ToolCreditConfig.insertMany(TOOL_COST_SEED, { ordered: false }).catch(() => {});
}

// Any tool whose scheduled effective date has now arrived gets its cost
// promoted from scheduledCost -> cost, and a notification is created so
// users see it on the bell icon. Runs at most once per cache refresh
// (every ~30s), so this is cheap even with many tools.
async function promoteScheduledToolCosts(docs) {
  const now = new Date();
  const due = docs.filter((d) => d.effectiveAt && d.scheduledCost != null && now >= new Date(d.effectiveAt));
  if (due.length === 0) return docs;

  for (const doc of due) {
    await ToolCreditConfig.updateOne(
      { _id: doc._id },
      {
        $set: {
          cost: doc.scheduledCost,
          minCost: doc.scheduledMinCost ?? doc.minCost,
        },
        $unset: { scheduledCost: "", scheduledMinCost: "", effectiveAt: "" },
      }
    );
    await Notification.create({
      title: "Credit cost updated",
      message: `${doc.label} now costs ${doc.scheduledCost} credit${doc.scheduledCost === 1 ? "" : "s"} per use.`,
    }).catch(() => {});
  }

  return ToolCreditConfig.find({}).lean();
}

/**
 * Returns the current tool cost configs as a { [toolId]: config } map,
 * reading from MongoDB (with a short cache, same pattern as getPlans()).
 */
export async function getToolCreditConfigs() {
  const now = Date.now();
  if (toolCostCache.costs && now - toolCostCache.loadedAt < CACHE_TTL_MS) {
    return toolCostCache.costs;
  }

  try {
    await dbConnect();
    await seedToolCostsIfEmpty();
    let docs = await ToolCreditConfig.find({}).lean();
    docs = await promoteScheduledToolCosts(docs);

    const map = {};
    for (const doc of docs) map[doc.toolId] = doc;
    toolCostCache.costs = map;
    toolCostCache.loadedAt = now;
    return map;
  } catch (err) {
    console.error("getToolCreditConfigs: falling back to seed defaults —", err.message);
    const fallback = {};
    for (const seed of TOOL_COST_SEED) fallback[seed.toolId] = seed;
    return fallback;
  }
}

/** Invalidate the tool-cost cache immediately — call this after any admin write. */
export function invalidateToolCostsCache() {
  toolCostCache.costs = null;
  toolCostCache.loadedAt = 0;
}
...
export async function creditCostForTool(toolId, context = {}) {
  ...
  if (tool.costType === "per-length") {
    ...
    const units = Math.ceil(text.length / (tool.unitSize || 200));
    return Math.max(tool.minCost || 0, units * (tool.cost || 1));
  }
...
export async function getToolCreditState(user, toolId, now = new Date(), context = {}) {
  ...
  const cost = await creditCostForTool(toolId, context);
  return {
