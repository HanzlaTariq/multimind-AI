...
const COLORS = {
  ...
  divider: [226, 232, 240], // slate-200, turn separators
};
...
function computeColWidths(header, rows, totalWidth) {
  ...
  const minWidth = 46;
  let widths = lens.map((l) => Math.max(minWidth, (l / totalLen) * totalWidth));
  const sum = widths.reduce((a, b) => a + b, 0);
  widths = widths.map((w) => (w * totalWidth) / sum);
  // Fix any rounding drift so columns exactly fill totalWidth
  const drift = totalWidth - widths.reduce((a, b) => a + b, 0);
  widths[colCount - 1] += drift;
  return widths;
}

async function renderTableBlock(doc, block, startY) {
  const { header, rows } = block;
  const widths = computeColWidths(header, rows, CONTENT_WIDTH);
  const cellPadX = 6;
  const lineHeight = 12;
  const fontSize = 9;
  let y = startY;

  async function wrapRow(cells, bold) {
    const wrapped = [];
    let maxLines = 1;
    for (let i = 0; i < widths.length; i++) {
      const { text, isArabic } = await setTextStyle(doc, cells[i] || "", {
        size: fontSize,
        bold,
      });
      const w = Math.max(10, widths[i] - cellPadX * 2);
      const lines = doc.splitTextToSize(text || " ", w);
      wrapped.push({ lines, isArabic });
      maxLines = Math.max(maxLines, lines.length);
    }
    return { wrapped, height: maxLines * lineHeight + 10 };
  }

  function drawCells(rowY, wrapped, height, textColor) {
    let x = MARGIN_X;
    for (let i = 0; i < wrapped.length; i++) {
      const { lines, isArabic } = wrapped[i];
      doc.setTextColor(...textColor);
      let cy = rowY + 14;
      for (const line of lines) {
        if (isArabic) {
          doc.text(line, x + widths[i] - cellPadX, cy, { align: "right" });
        } else {
  ...
  function drawGrid(rowY, height) {
    doc.setDrawColor(226, 232, 240);
    doc.setLineWidth(0.5);
    let x = MARGIN_X;
    for (let i = 0; i <= widths.length; i++) {
      doc.line(x, rowY, x, rowY + height);
      if (i < widths.length) x += widths[i];
    }
    doc.line(MARGIN_X, rowY + height, MARGIN_X + CONTENT_WIDTH, rowY + height);
  }

  function drawHeaderRow(top, wrapped, height) {
    doc.setFillColor(...COLORS.bannerBg);
    doc.rect(MARGIN_X, top, CONTENT_WIDTH, height, "F");
    drawCells(top, wrapped, height, [255, 255, 255]);
    doc.setDrawColor(...COLORS.accent);
    doc.setLineWidth(1.2);
    doc.line(MARGIN_X, top, MARGIN_X + CONTENT_WIDTH, top);
    doc.setLineWidth(0.5);
  }

  const { wrapped: headerWrapped, height: headerHeight } = await wrapRow(header, true);
  if (y + headerHeight > PAGE.height - MARGIN_BOTTOM) y = addPage(doc);
  drawHeaderRow(y, headerWrapped, headerHeight);
  y += headerHeight;

  for (let r = 0; r < rows.length; r++) {
    const { wrapped, height } = await wrapRow(rows[r], false);

    if (y + height > PAGE.height - MARGIN_BOTTOM) {
      y = addPage(doc);
      drawHeaderRow(y, headerWrapped, headerHeight);
      y += headerHeight;
    }

    doc.setFillColor(...(r % 2 === 0 ? [255, 255, 255] : COLORS.codeBg));
    doc.rect(MARGIN_X, y, CONTENT_WIDTH, height, "F");
    drawCells(y, wrapped, height, COLORS.text);
    drawGrid(y, height);
    y += height;
  }
...
async function renderBlocks(doc, blocks, startY) {
  ...
  for (const block of blocks) {
    if (block.type === "table") {
      y = await renderTableBlock(doc, block, y);
      continue;
    }
...
export async function exportTextToPdf(rawText, filename = "multimind-export.pdf") {
  ...
  const avatarR = 9;
  drawAvatar(doc, MARGIN_X, y + avatarR, avatarR, "M");
  doc.setFont("helvetica", "bold");
  doc.setFontSize(9.5);
  doc.setTextColor(...COLORS.heading);
  doc.text("MultiMind", MARGIN_X + avatarR * 2 + 8, y + avatarR + 3);
  y += avatarR * 2 + 12;
...
function drawAvatar(doc, x, centerY, r, letter) {
  doc.setFillColor(...COLORS.accent);
  doc.circle(x + r, centerY, r, "F");
  doc.setFont("helvetica", "bold");
  doc.setFontSize(r + 1);
  doc.setTextColor(...COLORS.bannerBg);
  doc.text(letter, x + r, centerY + r * 0.35, { align: "center" });
}
...
export async function exportConversationToPdf(turns, title = "MultiMind Conversation") {
  ...
  for (let i = 0; i < turns.length; i++) {
    ...
    // ---- Question: right-aligned chat bubble, sized to its content ----
    const { text: qText, isArabic: qIsArabic } = await setTextStyle(doc, turn.prompt, {
      size: 10.5,
    });
    const qLines = doc.splitTextToSize(qText, BUBBLE_MAX_WIDTH - 24);
    let bubbleW = 120;
    for (const line of qLines) {
      bubbleW = Math.max(bubbleW, doc.getTextWidth(line) + 24);
    }
    bubbleW = Math.min(bubbleW, BUBBLE_MAX_WIDTH);
    const qHeight = qLines.length * 14 + 18;
    const labelHeight = 16;

    if (y + labelHeight + qHeight > PAGE.height - MARGIN_BOTTOM) {
      y = addPage(doc);
    }
    ...
    doc.setFontSize(8.5);
    doc.setTextColor(...COLORS.muted);
    doc.text("You", PAGE.width - MARGIN_X, y, { align: "right" });
    y += labelHeight;

    const bubbleX = PAGE.width - MARGIN_X - bubbleW;
    doc.setFillColor(...COLORS.bubbleBg);
    doc.roundedRect(bubbleX, y, bubbleW, qHeight, 8, 8, "F");
    doc.setTextColor(...COLORS.heading);
    let qy = y + 17;
    for (const line of qLines) {
      if (qIsArabic) {
        doc.text(line, bubbleX + bubbleW - 12, qy, { align: "right" });
      } else {
        doc.text(line, bubbleX + 12, qy);
      }
      qy += 14;
    }
    y += qHeight + 18;

    // ---- Answer: avatar + label, then the rendered content ----
    const avatarR = 9;
    if (y + 20 > PAGE.height - MARGIN_BOTTOM) y = addPage(doc);
    drawAvatar(doc, MARGIN_X, y + avatarR, avatarR, "M");
    doc.setFont("helvetica", "bold");
    doc.setFontSize(9.5);
    doc.setTextColor(...COLORS.heading);
    ...
    // ---- Divider before the next turn ----
    if (i < turns.length - 1) {
      y += 8;
      if (y + 20 > PAGE.height - MARGIN_BOTTOM) {
        y = addPage(doc);
      } else {
